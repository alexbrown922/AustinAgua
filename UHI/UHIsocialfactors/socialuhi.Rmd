...
#st_write(census_social, "austin_uhi_census.shp", driver="ESRI Shapefile")
#c_s_test = st_read(dsn = ".", layer = "austin_uhi_census") %>%
  #st_transform(crs = 4326)
...
```{r}
mult_uhi = lm(uhi.x~median_income+nonwhite+tree_intensity, data = racial_makeup)
summary(mult_uhi)

mult_ti=lm(tree_intensity~median_income+nonwhite, data = racial_makeup)
summary(mult_ti)

```

```{r, results='asis'}
stargazer(mult_ti,  type = "html")
```