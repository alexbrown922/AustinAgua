...
census_social = full_join(census_tree, social_prior, by="shape_area") %>% 
  dplyr::select(geoid10.y, air_qual.x, critical_p.x, district.x, ej.x, health.x, preserve.x, replenish.x, score.x, shape_area, uhi.x, water_qual.x, tree_sum, total_population, median_income, commuting, x62andover, white, black, native_am, asian, pacific_islander, latinx) %>% 
...
ggplot(racial_makeup)+
  ...
  geom_text(x = 0.10, y = 0.2, aes(label = "*** R Squared = 0.11"))+
  scale_x_continuous()
```
...
ggplot(racial_makeup)+
  ...
  geom_text(x = 1.0e-07, y = 0.25, aes(label = "*** R Squared = 0.23"))+
  scale_x_continuous()
```
