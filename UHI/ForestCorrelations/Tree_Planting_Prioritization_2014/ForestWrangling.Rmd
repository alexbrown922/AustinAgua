...
tree_plot_value = ggplot(tree_value)+
  ...
  scale_fill_gradient2("Value Discrepency",low = "firebrick4", mid = "white", high = "deepskyblue4", breaks=c(min(tree_value$value_disc),0,max(tree_value$value_disc)),labels=c("-$10,000","$0","$14,000"))+
...
tree_plot_waller_value = ggplot(tree_value_waller)+
  geom_sf(aes(fill = value_disc))+
  scale_fill_gradient2("Value Discrepency",low = "firebrick4", mid = "white", high = "deepskyblue4", breaks=c(min(tree_value_waller$value_disc),0,max(tree_value_waller$value_disc)),labels=c("-$400","$0","$250"))+
  #scale_colour_brewer(palette = "PiYG")+
  labs(x = "", y = "", title = "Tree Value Discrepency Based on Simulated Tree \nIntensity By Census Tract")+
  geom_sf(data = Wallercreek, 
          fill = "NA", 
