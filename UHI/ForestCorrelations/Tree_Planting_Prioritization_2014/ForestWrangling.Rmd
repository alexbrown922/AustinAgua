...
tree_value = tree_count %>% 
  ...
  mutate(tree_ben = tree_sum*value_per_tree)
```
...
tree_plot_benefit = ggplot(tree_value)+
  ...
  scale_fill_gradient2("Value Discrepency",low = "white", high = "deepskyblue4", breaks=c(min(tree_value$tree_ben),max(tree_value$tree_ben)),labels=c("$0","$34,000"))+
...
tree_plot_waller_value = ggplot(tree_value_waller)+
  geom_sf(aes(fill = value_disc))+
  scale_fill_gradient2("Value Discrepency",low = "firebrick4", mid = "white", high = "deepskyblue4", breaks=c(min(tree_value_waller$value_disc),0,max(tree_value_waller$value_disc)),labels=c("-$500","$0","$350"))+
  #scale_colour_brewer(palette = "PiYG")+
  labs(x = "", y = "", title = "Tree Value Discrepency Based on Simulated Tree \nIntensity By Census Tract")+
  geom_sf(data = Wallercreek, 
          fill = "NA", 
