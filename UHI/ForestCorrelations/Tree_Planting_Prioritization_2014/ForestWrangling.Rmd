...
tree_count_only = tree_count %>% 
  dplyr::select(fid, uhi, tree_numbe, tree_sum) %>% 
  as.data.frame()
```
...
tree_plot = ggplot(tree_count)+
  geom_sf(aes(fill = tree_numbe))+
  scale_fill_gradient("Simulated Trees", low = "lightyellow", high = "darkgreen")+
  theme_minimal()+
  coord_sf(datum=NA)+
  labs(x = "", y = "", title = "Simulated Tree Canopy By Census Track")
...
uhi_regress = lm(uhi~tree_numbe, data = tree_count_only)
#summary(uhi_regress)

ggplot(tree_count_only)+
  geom_point(aes(x = tree_numbe,y=uhi))+
  theme_classic()+
  labs(title = "Relationship Between Urban Heat Island and Simulated \nTree Canopy by Austin Census Tract", y = "Comparitive Land Surface Temperature Intensity", x = "Simulated Tree Count")+
  geom_smooth(method = "lm", aes(x = tree_numbe,y=uhi))+
  geom_text(x = 775, y = 0.58, aes(label = "*** R Squared = 0.083"))
